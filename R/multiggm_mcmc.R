...
#'
#' Thin R wrapper that calls the C++ MCMC engine. Falls back to pure R
#' if \code{engine = "R"}.
#'
#' @keywords internal
.multiggm_mcmc_single <- function(S_list, n_vec, burnin, nsave, thin,
  ...
  for (iter in seq_len(niter)) {
    ...
    for (cur_graph in seq_len(K)) {
      ...
      for (ii in 1:(p - 1)) {
        for (jj in (ii + 1):p) {
          logH <- log_H_cpp(b_prior_vec[cur_graph], D_prior_arr[, , cur_graph],
                            n_vec[cur_graph], S_arr[, , cur_graph], Ck, ii, jj)
          sim_term <- nu[ii, jj] + 2 * sum(Theta[cur_graph, ] * as.numeric(adj_arr[ii, jj, ]))
          w_logodds <- logH - sim_term
          if (is.na(w_logodds)) next
          if (is.infinite(w_logodds)) {
            w <- if (w_logodds > 0) 0 else 1
          } else if (w_logodds > 0) {
            ew <- exp(-w_logodds); w <- ew / (1 + ew)
          } else {
            ew <- exp(w_logodds); w <- 1 / (1 + ew)
          }
          ...
          if (isTRUE(propose_ij != current_ij)) {
            out_prior <- GWishart_NOij_Gibbs_cpp(b_prior_vec[cur_graph], D_prior_arr[, , cur_graph],
                                                  adjk * 1L, Ck, ii, jj, as.integer(propose_ij), 0L, 1L)
            if (isFALSE(out_prior$ok)) next
            C_prop <- out_prior$C
            r2 <- log_GWishart_NOij_pdf_cpp(b_prior_vec[cur_graph], D_prior_arr[, , cur_graph],
                                             C_prop, ii, jj, as.integer(current_ij)) -
              log_GWishart_NOij_pdf_cpp(b_prior_vec[cur_graph], D_prior_arr[, , cur_graph],
                                        C_prop, ii, jj, as.integer(propose_ij))
            if (log(stats::runif(1)) < r2) {
              adjk[ii, jj] <- propose_ij; adjk[jj, ii] <- propose_ij; current_ij <- propose_ij
            }
          }
          out_g <- GWishart_NOij_Gibbs_cpp(b_post[cur_graph], D_post_arr[, , cur_graph],
                                            adjk * 1L, Ck, ii, jj, as.integer(current_ij), 0L, 0L)
          if (isFALSE(out_g$ok)) next
          Ck <- out_g$C; adjk <- out_g$adj
        }
      }
      out_bips <- GWishart_BIPS_maximumClique_cpp(b_post[cur_graph], D_post_arr[, , cur_graph],
                                                    adjk * 1L, Ck, 0L, 1L)
      if (isFALSE(out_bips$ok)) next
      C_arr[, , cur_graph] <- out_bips$C; adj_arr[, , cur_graph] <- adjk
    }
    ...
    for (k in 1:(K - 1)) {
      for (m in (k + 1):K) {
        ...
        Theta_prop <- Theta; Theta_prop[k, m] <- theta_prop; Theta_prop[m, k] <- theta_prop
        sum_over_edges <- 0
        for (ii in 1:(p - 1)) for (jj in (ii + 1):p)
          sum_over_edges <- sum_over_edges + calc_mrf_logC(Theta, nu[ii, jj]) +
            2 * (theta_prop - theta_curr) * as.numeric(adj_arr[ii, jj, k]) * as.numeric(adj_arr[ii, jj, m]) -
        ...
        log_q_fwd <- if (theta_curr == 0) stats::dgamma(theta_prop, shape = alpha_prop, rate = beta_prop, log = TRUE) else 0
        log_q_rev <- if (theta_prop == 0) stats::dgamma(theta_curr, shape = alpha_prop, rate = beta_prop, log = TRUE) else 0
        log_ar <- (log_prior_prop - log_prior_curr) + sum_over_edges + (log_q_rev - log_q_fwd)
        if (is.finite(log_ar) && log_ar > log(stats::runif(1))) {
          Theta[k, m] <- theta_prop; Theta[m, k] <- theta_prop
          ar_gamma[k, m] <- ar_gamma[k, m] + 1 / niter
        }
        if (Theta[k, m] != 0) {
          theta_curr2 <- Theta[k, m]; theta_prop2 <- stats::rgamma(1, shape = alpha_prop, rate = beta_prop)
          n_within_model[k, m] <- n_within_model[k, m] + 1
          Theta_prop2 <- Theta; Theta_prop2[k, m] <- theta_prop2; Theta_prop2[m, k] <- theta_prop2
          se2 <- 0
          for (ii in 1:(p - 1)) for (jj in (ii + 1):p)
            se2 <- se2 + calc_mrf_logC(Theta, nu[ii, jj]) + 2 * (theta_prop2 - theta_curr2) *
              as.numeric(adj_arr[ii, jj, k]) * as.numeric(adj_arr[ii, jj, m]) - calc_mrf_logC(Theta_prop2, nu[ii, jj])
          log_theta_ar <- (stats::dgamma(theta_prop2, shape = alpha, rate = beta, log = TRUE) -
                             stats::dgamma(theta_curr2, shape = alpha, rate = beta, log = TRUE)) + se2 +
            stats::dgamma(theta_curr2, shape = alpha_prop, rate = beta_prop, log = TRUE) -
          ...
          if (is.finite(log_theta_ar) && log_theta_ar > log(stats::runif(1))) {
            Theta[k, m] <- theta_prop2; Theta[m, k] <- theta_prop2; ar_theta[k, m] <- ar_theta[k, m] + 1
          }
        }
      }
    }
    ...
    for (ii in 1:(p - 1)) {
      for (jj in (ii + 1):p) {
        ...
        log_nu_ar <- (nu_prop - nu[ii, jj]) * (sum(adj_arr[ii, jj, ]) + a - a_prop) -
          (a + b - a_prop - b_prop) * log(1 + exp(nu_prop)) - calc_mrf_logC(Theta, nu_prop) +
          (a + b - a_prop - b_prop) * log(1 + exp(nu[ii, jj])) + calc_mrf_logC(Theta, nu[ii, jj])
        if (log_nu_ar > log(stats::runif(1))) {
          nu[ii, jj] <- nu_prop; nu[jj, ii] <- nu_prop; ar_nu[ii, jj] <- ar_nu[ii, jj] + 1 / niter
        }
      }
    }
    ...
    if (iter > burnin && ((iter - burnin) %% thin == 0L)) {
      ...
      if (save_idx <= nsave) {
        ...
        for (k in seq_len(K)) Sig_save[, , k, save_idx] <- solve(C_arr[, , k])
        adj_save[, , , save_idx] <- adj_arr * 1L
        Theta_save[, , save_idx] <- Theta; nu_save[, , save_idx] <- nu
      }
    }
  }

  for (k in 1:(K - 1)) for (m in (k + 1):K)
